...
public class DepartmentDAOImpl extends DAO implements DepartmentDAO {
	...
	public Department selectOne(int departmentId) {
		...
		try {
			...
			if(rs.next()) {
				...
				dept.setDepartmentId(rs.getInt("department_id"));
				dept.setDepartmentName(rs.getString("department_name"));
				dept.setManagerId(rs.getString("manger_id"));
				dept.setLocationId(rs.getInt("location_id"));
			}
		}catch(SQLException e) {
			e.printStackTrace();
	...
	public void insert(Department dept) {
		try {
			connect();
			String insert = "INSERT * FROM departments VALEUS(?,?,?,?)";
	...
	public void update(Department dept) {
		try {
			connect();
			String update = "UPDATE * FROM departments SET department_name = ? WHERE department_id = ?";
	...
	public void delete(int departmentId) {
		try {
			...
			System.out.println(reuslt + "건이 삭제되었습니다.");
